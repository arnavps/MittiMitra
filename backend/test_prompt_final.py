...
def test_language(lang, file):
    ...
    if lang == "English":
        if "Ji Kisan Bhai" in prompt:
            file.write("FAIL: Found Hindi greeting in English prompt!\n")
    ...
    if lang == "Hindi":
        if "Ji Kisan Bhai" not in prompt:
            file.write("FAIL: Missing Hindi greeting in Hindi prompt!\n")
...
if __name__ == "__main__":
    with io.open("test_results_final.txt", "w", encoding="utf-8") as f:
        test_language("English", f)
        test_language("Hindi", f)
    print("Test complete. Results in test_results_final.txt")
